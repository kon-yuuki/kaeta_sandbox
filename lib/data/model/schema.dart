...
const ps.Schema schema = ps.Schema([
  ps.Table('items', [
    ...
    ps.Column.integer(
      'quantity_unit',
    ), // 用途: ほしい量の単位 / 値: 0=g, 1=mg, 2=ml, 3=kg, 4=L, null
  ...
  ps.Table('todo_items', [
    ...
    ps.Column.text(
      'category_id',
    ), // 用途: 登録時の categories.id / 値: UUID文字列 or null
    ...
    ps.Column.integer(
      'budget_min_amount',
    ), // 用途: 登録時点の予算下限 / 値: 200, 500... or null
    ps.Column.integer(
      'budget_max_amount',
    ), // 用途: 登録時点の予算上限 / 値: 350, 1000... or null
    ...
    ps.Column.integer(
      'quantity_unit',
    ), // 用途: 登録時点のほしい量の単位 / 値: 0=g, 1=mg, 2=ml, 3=kg, 4=L, null
  ...
  ps.Table('profiles', [
    ps.Column.text(
      'current_family_id',
    ), // 用途: 現在選択中の家族 / 値: families.id or null
    ...
    ps.Column.text(
      'avatar_preset',
    ), // 用途: プリセット画像キー / 値: "assets/avatar/a.png" or null
  ...
  ps.Table('family_boards', [
    ...
    ps.Column.text(
      'updated_by',
    ), // 用途: 最終更新者のユーザー識別 / 値: auth user id(UUID) or null
  ...
  ps.Table('app_notification_reactions', [
    ps.Column.text(
      'event_id',
    ), // 用途: 対象通知イベント識別子 / 値: app_notifications.event_id
...
class Items extends Table {
  ...
  TextColumn get categoryId => text().nullable().references(
    Categories,
    #id,
  )(); // 用途: categories.id 参照 / 値: UUID or null
  TextColumn get reading => text()(); // 用途: かな検索キー / 値: "ぎゅうにゅう"
  IntColumn get purchaseCount =>
      integer().withDefault(const Constant(0))(); // 用途: 購入頻度集計 / 値: 0,1,2...
  TextColumn get userId =>
      text().nullable()(); // 用途: 作成ユーザー識別 / 値: auth user id or null
  TextColumn get familyId =>
      text().nullable()(); // 用途: 家族共有範囲 / 値: families.id or null
  TextColumn get imageUrl =>
      text().nullable()(); // 用途: 商品画像URL / 値: https URL or null
  IntColumn get budgetMinAmount =>
      integer().nullable()(); // 用途: 予算下限 / 値: 200,500... or null
  IntColumn get budgetMaxAmount =>
      integer().nullable()(); // 用途: 予算上限 / 値: 350,1000... or null
  IntColumn get budgetType =>
      integer().nullable()(); // 用途: 予算単位 / 値: 0=1つあたり,1=100gあたり
  TextColumn get quantityText =>
      text().nullable()(); // 用途: ほしい量の自由入力 / 値: "2","500",null
  IntColumn get quantityUnit =>
      integer().nullable()(); // 用途: ほしい量の単位 / 値: 0=g,1=mg,2=ml,3=kg,4=L,null
  IntColumn get quantityCount =>
      integer().nullable()(); // 用途: ほしい個数 / 値: 1,2... or null
...
class Categories extends Table {
  TextColumn get id =>
      text().clientDefault(() => const Uuid().v4())(); // 用途: カテゴリ主キー / 値: UUID
...
class TodoItems extends Table {
  TextColumn get id => text().clientDefault(
    () => const Uuid().v4(),
  )(); // 用途: 買い物リスト行の主キー / 値: UUID
  TextColumn get itemId => text().nullable().references(
    Items,
    #id,
  )(); // 用途: items.id 参照 / 値: UUID or null
  ...
  TextColumn get categoryId => text().nullable().references(
    Categories,
    #id,
  )(); // 用途: 作成時の categories.id / 値: UUID or null
  BoolColumn get isCompleted => boolean().withDefault(
    const Constant(false),
  )(); // 用途: 購入済み状態 / 値: false=未購入,true=購入済み
  IntColumn get priority =>
      integer().withDefault(const Constant(0))(); // 用途: 優先度 / 値: 0,1,2...
  ...
  )(); // 用途: 作成ユーザー識別 / 値: auth user id or null
  IntColumn get budgetMinAmount =>
      integer().nullable()(); // 用途: 登録時予算下限 / 値: 200,500... or null
  IntColumn get budgetMaxAmount =>
      integer().nullable()(); // 用途: 登録時予算上限 / 値: 350,1000... or null
  IntColumn get budgetType =>
      integer().nullable()(); // 用途: 登録時予算単位 / 値: 0=1つあたり,1=100gあたり
  DateTimeColumn get completedAt =>
      dateTime().nullable()(); // 用途: 完了日時 / 値: DateTime or null
  TextColumn get quantityText =>
      text().nullable()(); // 用途: 登録時ほしい量の自由入力 / 値: "2","500",null
  IntColumn get quantityUnit =>
      integer().nullable()(); // 用途: 登録時ほしい量の単位 / 値: 0=g,1=mg,2=ml,3=kg,4=L,null
  IntColumn get quantityCount =>
      integer().nullable()(); // 用途: 登録時ほしい個数 / 値: 1,2... or null
...
class PurchaseHistory extends Table {
  TextColumn get id =>
      text().clientDefault(() => const Uuid().v4())(); // 用途: 履歴行の主キー / 値: UUID
  TextColumn get itemId => text().nullable().references(
    Items,
    #id,
  )(); // 用途: items.id 参照 / 値: UUID or null
  ...
  DateTimeColumn get lastPurchasedAt =>
      dateTime()(); // 用途: 最終購入日時 / 値: DateTime
  TextColumn get userId =>
      text().nullable()(); // 用途: 記録ユーザー識別 / 値: auth user id or null
...
class Profiles extends Table {
  ...
  TextColumn get currentFamilyId =>
      text().nullable()(); // 用途: 現在選択中の家族 / 値: families.id or null
  ...
  BoolColumn get onboardingCompleted => boolean().withDefault(
    const Constant(false),
  )(); // 用途: 初期設定完了状態 / 値: false/true
  TextColumn get avatarPreset =>
      text().nullable()(); // 用途: プリセット画像キー / 値: asset path or null
  TextColumn get avatarUrl =>
      text().nullable()(); // 用途: カスタム画像URL / 値: https URL or null
...
class Families extends Table {
  TextColumn get id =>
      text().clientDefault(() => const Uuid().v4())(); // 用途: 家族主キー / 値: UUID
...
class FamilyMembers extends Table {
  TextColumn get id => text().clientDefault(
    () => const Uuid().v4(),
  )(); // 用途: 家族メンバー行の主キー / 値: UUID
  TextColumn get userId => text()(); // 用途: メンバーのユーザー識別 / 値: auth user id
  TextColumn get familyId =>
      text().references(Families, #id)(); // 用途: 所属家族識別 / 値: families.id
  DateTimeColumn get createdAt => dateTime().clientDefault(
    () => DateTime.now(),
  )(); // 用途: 参加日時 / 値: DateTime
...
class MasterItems extends Table {
  TextColumn get id =>
      text().clientDefault(() => const Uuid().v4())(); // 用途: マスタ商品主キー / 値: UUID
...
class Invitations extends Table {
  ...
  TextColumn get familyId =>
      text().references(Families, #id, onDelete: KeyAction.cascade)();
...
class FamilyBoards extends Table {
  TextColumn get id =>
      text().clientDefault(() => const Uuid().v4())(); // 用途: ボード行の主キー / 値: UUID
  TextColumn get familyId =>
      text().nullable()(); // 用途: 家族ボード対象 / 値: families.id or null(個人メモ)
  TextColumn get userId =>
      text().nullable()(); // 用途: 個人メモ所有者 / 値: auth user id or null
  TextColumn get message =>
      text().withDefault(const Constant(''))(); // 用途: ボード本文 / 値: 任意文字列
  TextColumn get updatedBy =>
      text().nullable()(); // 用途: 最終更新者のユーザー識別 / 値: auth user id or null
  DateTimeColumn get updatedAt => dateTime().clientDefault(
    () => DateTime.now(),
  )(); // 用途: 最終更新日時 / 値: DateTime
...
class AppNotifications extends Table {
  TextColumn get id =>
      text().clientDefault(() => const Uuid().v4())(); // 用途: 通知主キー / 値: UUID
  TextColumn get message => text()(); // 用途: 通知メッセージ / 値: "牛乳を追加しました"
  TextColumn get title =>
      text().nullable()(); // 用途: 通知タイトル / 値: "リストにアイテムが追加されました"
  TextColumn get body =>
      text().nullable()(); // 用途: 通知本文 / 値: "かつまたさんが2個のアイテムを追加しました"
  IntColumn get type => integer().withDefault(
    const Constant(0),
  )(); // 用途: 通知タイプ / 値: 0=通常, 1=買い物完了
  BoolColumn get isRead => boolean().withDefault(
    const Constant(false),
  )(); // 用途: 既読状態 / 値: false=未読, true=既読
  DateTimeColumn get createdAt => dateTime().clientDefault(
    () => DateTime.now(),
  )(); // 用途: 作成日時 / 値: DateTime
  TextColumn get userId => text()(); // 用途: 通知対象ユーザー / 値: auth user id
  TextColumn get actorUserId =>
      text().nullable()(); // 用途: 通知の実施者ユーザー / 値: auth user id
  TextColumn get eventId => text().nullable()(); // 用途: 同一通知イベント識別子 / 値: UUID
  TextColumn get reactionEmoji =>
      text().nullable()(); // 用途: 通知への絵文字リアクション / 値: "👍" など or null
  TextColumn get familyId =>
      text().nullable()(); // 用途: 家族スコープ / 値: families.id or null(個人)
...
class AppNotificationReactions extends Table {
  ...
  TextColumn get eventId =>
      text()(); // 用途: 対象通知イベント識別子 / 値: app_notifications.event_id
