...
Future<void> ensureProfile() async {
  ...
  // 1. ローカルをチェック
  final localProfile = await (db.select(db.profiles)..where((t) => t.id.equals(userId))).getSingleOrNull();
  if (localProfile != null) return; // すでにあれば何もしない

  // 2. サーバーをチェック
  final remoteData = await supabase.from('profiles').select().eq('id', userId).maybeSingle();
