...
| P7 | A | 2026-02-19 | Pass（条件付き） | `https` 招待リンクはブラウザ遷移後に「ページを開けません」で失敗。`開けない方はこちら` のリンクをタップすると `参加してはじめる` に直接遷移し、押下時に「すでに別のチームに参加しています。」スナックバー表示（参加拒否は期待どおり）。 |
...
| P3 | A | 2026-02-22 | Pass（フォールバック導線） | `https://kaeta-jointeam.com/invite/<inviteId>` をSafariで開くとWebフォールバック表示。`アプリで開く` 押下後はKaetaへ遷移し、通常どおり参加完了。直接Universal Link起動（ブラウザ非経由）は未達。 |
| P4a | A | 2026-02-22 | Pass（フォールバック導線） | 未インストール端末で招待リンクを開き、ブラウザからTestFlightへ遷移してインストール。再度リンクタップ→`アプリで開く`→`参加してはじめる`→`すでにアカウントをお持ちの方` でログイン後、正常に招待参加できた。 |
| P5 | A | 2026-02-22 | Pass | 未インストール・アカウントなしユーザーで招待リンクから開始。招待された人用オンボーディングを経由し、最終的に招待チームへ参加できた。 |
...
- 追加で確認された不具合:
  ...
  - P7再検証で、`https` 招待リンクはブラウザで開けないケースを確認。ただしフォールバックリンク（`開けない方はこちら`）では期待どおり参加拒否まで遷移可能。
  - DNS復旧後（2026-02-22）、Safariで `https` 招待リンク自体は正常表示。`アプリで開く` で参加成功する一方、直接Universal Link起動（ブラウザ非経由）は継続未達。
  - P4aを再検証し、未インストール→TestFlight導線→既存アカウントログイン後の招待参加完了を確認（2026-02-22）。
- 2026-02-22 追加確認ログ:
  - ドメイン復旧確認:
    - `whois kaeta-jointeam.com` で `Domain Status: ok` を確認（`clientHold` 解消）。
    - `dig NS kaeta-jointeam.com +short` で `emma.ns.cloudflare.com` / `kipp.ns.cloudflare.com` を確認。
    - `dig A kaeta-jointeam.com +short` で `172.67.139.248` / `104.21.54.15` を確認。
  - Workers疎通確認:
    - `curl -I https://kaeta-jointeam.com/invite/test` は到達し、UUID不正として `400 Bad Request`（期待挙動）を確認。
  - AASA配信確認:
    - `curl -sS https://kaeta-jointeam.com/.well-known/apple-app-site-association`
    - 返却JSONに `appID: 8MKVD3TZQ3.com.kon-yuuki.kaetaSandbox` と `paths: [\"/invite/*\"]` を確認（設定一致）。
  - Safe Browsing再確認:
    - Chromeで `https://kaeta-jointeam.com/invite/<有効inviteId>` を開けることを確認（赤警告なし）。
  - 実機導線確認:
    - ブラウザ表示→`アプリで開く` は成功し、参加完了。
    - 直接Universal Link起動（ブラウザ非経由）は未達（継続課題）。
- 対応状況:
  ...
  - DNS調査結果（2026-02-19）:
    - `dig NS kaeta-jointeam.com @a.gtld-servers.net` が `NXDOMAIN`。
    - `.com` レジストリ側でドメイン委任が見えておらず、Cloudflare/Worker以前の段階で名前解決不可。
    - 本日設定反映の可能性があるため、最大48時間の浸透待ちを実施。
- 未完了タスク:
  ...
  - Universal Link直起動（ブラウザ非経由）の達成確認（AASA一致は確認済みのため、実機再インストール・配信経路別タップで継続検証）。
  - 実機確認（方針どおり動くか）:
    - 既存アカウント/オンボ完了: 参加導線でログイン後に参加完了（再オンボなし）
    - 既存アカウント/オンボ未完了: 参加用オンボ（チーム名・招待ステップなし）経由で参加
    - 新規アカウント: 参加用オンボ（チーム名・招待ステップなし）経由で参加

### 5.1 DNS浸透待ち後の再開チェック手順
1. レジストリ直確認（最優先）
   - `dig NS kaeta-jointeam.com @a.gtld-servers.net`
   - 期待: `NXDOMAIN` ではなく `emma.ns.cloudflare.com` / `kipp.ns.cloudflare.com` が返る
2. 通常リゾルバ確認
   - `dig NS kaeta-jointeam.com +short`
   - `dig A kaeta-jointeam.com +short`
   - 期待: NSがCloudflareの2本、Aが1件以上
3. ブラウザ疎通確認（Safari）
   - `https://kaeta-jointeam.com/.well-known/apple-app-site-association`
   - `https://kaeta-jointeam.com/invite/<有効inviteId>`
   - 期待: 「サーバが見つかりません」にならない
4. Safe Browsing確認
   - Search Consoleで所有権確認完了 → 必要なら再審査申請
5. 実機再検証
   - P3 / P4a / P7 をA/Bで再実施し、ログ更新

買い物メモアプリで一緒にリストを共有しましょう！
こちらのリンクから家族グループ「渡辺」に参加できます。

https://kaeta-jointeam.com/invite/5b6511e8-9498-4962-b7e5-db8556c85b68

開けない場合: kaeta://invite/5b6511e8-9498-4962-b7e5-db8556c85b68
