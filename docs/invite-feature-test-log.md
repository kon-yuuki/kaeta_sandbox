...
|---|---|---|---|---|
| U1（旧P1a/P1b） | A/B | 済み | アカウントあり・オンボ未完了・未ログイン（認証遷移前提） | `参加する` 後に認証へ遷移し、招待付きオンボを再開して参加完了できる |
...
|---|---|---|---|---|---|
| U1（旧P1a/P1b） | A | 2026-02-16 | Pass | 参考 | `チームに参加` クリック後に認証画面へ遷移。認証後、`チーム名設定` と `招待フロー` を省略した招待付きオンボーディングを経由して参加完了。 |
| U1（旧P1a/P1b） | B | 2026-02-16 | Pass | 有効 | 招待リンクから開始し、`参加する` 押下後に認証導線を経由。`チームを作成` ステップを省いたオンボーディング最終確認まで到達し、参加完了。 |
| S8 | A | 2026-02-16 | Pass | 有効 | 使用済みリンクをタップ後、招待エラーページへ遷移。期待どおり参加不可。 |
| S8 | B | 2026-02-16 | Pass | 有効 | アプリ停止（タスクキル）状態で使用済みリンクをタップ後、招待エラーページへ遷移。期待どおり参加不可。 |
