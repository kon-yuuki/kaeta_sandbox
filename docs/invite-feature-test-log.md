...
| S8 | A | 2026-02-16 | Fail | 一度使用された招待リンクを別ユーザーで開いた際、本来は無効招待エラーになるべきだが、現状は参加できてしまう。 |
| P7 | B | 2026-02-16 | Fail | リンククリック → ブラウザ起動 → 「このページをKaetaで開きますか？」モーダルで `開く` を押下 → アプリ遷移直後に `招待リンクエラー` 画面へ遷移。想定の `alreadyHasFamily` 分岐に到達しなかった。 |
