...
|---|---|---|---|---|---|
| P1a | 済み | あり | 未完了 | ログイン済み | 招待付きオンボーディングを継続して参加 |
| P1b | 済み | あり | 未完了 | 未ログイン | ログイン後に招待付きオンボーディングを継続して参加 |
| P2 | 済み | あり | 完了 | ログイン済み | すぐ参加確認して参加完了 |
...
### 4.2 パターン別フロー（現時点）
#### P1a: アプリインストール済み / アカウントあり / オンボーディング未完了 / ログイン済み（確認済み）
1. 招待リンク（`https://kaeta-jointeam.com/invite/{inviteId}`）をタップ
2. いったんブラウザへ遷移
3. ブラウザ上で「kaeta sandbox で開きますか？」ダイアログ表示
4. `はい` を押下するとアプリ起動
5. `参加してはじめる` ページへ遷移
6. `参加してはじめる` を押下すると `チームを作成して始める` ページへ遷移
7. 通常オンボーディング開始
8. 招待導線では `チーム名設定` と `招待フロー` はスキップされる
9. 初期設定完了後、招待されたチームに参加済み状態で開始

#### P1b: アプリインストール済み / アカウントあり / オンボーディング未完了 / 未ログイン（要確認）
1. 招待リンク（`https://kaeta-jointeam.com/invite/{inviteId}`）をタップ
2. ブラウザ経由でアプリ起動
3. ログイン画面へ遷移
4. ログイン成功後に招待コンテキストを復元
5. `参加してはじめる` ページへ遷移
6. `参加してはじめる` 押下で招待付きオンボーディング開始
7. 初期設定完了後、招待されたチームに参加済み状態で開始
...
## 8. 受け入れ確認シナリオ
### 8.1 P1a: アプリインストール済み / アカウントあり / オンボーディング未完了 / ログイン済み
1. `https://.../invite/{id}` タップでブラウザ経由後にアプリ起動できる。
2. `参加してはじめる` ページに遷移する。
3. `参加してはじめる` 押下で `チームを作成して始める` へ遷移する。
4. 招待導線では `チーム名設定` と `招待フロー` が表示されない。
5. 初期設定完了時に対象チームへ参加済みになる。

### 8.2 P1b: アプリインストール済み / アカウントあり / オンボーディング未完了 / 未ログイン
1. `https://.../invite/{id}` タップ後、ログイン画面へ遷移する。
2. ログイン後に招待情報が失われず、`参加してはじめる` ページへ復帰できる。
3. 招待付きオンボーディングを完了し、対象チームへ参加済みになる。

### 8.3 P2: アプリインストール済み / アカウントあり / オンボーディング完了 / ログイン済み
1. `https://.../invite/{id}` タップでアプリ起動できる。
2. 参加確認画面（または同等画面）で参加操作できる。
3. 参加後に対象チームがカレントチームになる。

### 8.4 P3: アプリインストール済み / アカウントあり / オンボーディング完了 / 未ログイン
1. `https://.../invite/{id}` タップ後、ログイン画面へ遷移する。
2. ログイン後に招待情報が失われず、参加フローに復帰できる。
3. 参加完了後に対象チームがカレントチームになる。

### 8.5 P4a: アプリ未インストール / アカウントあり / オンボーディング完了
1. `https://.../invite/{id}` タップでWebフォールバックが表示される。
2. TestFlight（公開後はApp Store）へ遷移できる。
3. インストール後初回起動でログインできる。
4. ログイン後に招待コンテキストを復元できる。
5. 参加確認画面を経由して参加完了できる。

### 8.6 P4b: アプリ未インストール / アカウントあり / オンボーディング未完了
1. `https://.../invite/{id}` タップでWebフォールバックが表示される。
2. TestFlight（公開後はApp Store）へ遷移できる。
3. インストール後初回起動でログインできる。
4. ログイン後に招待コンテキストを復元できる。
5. 招待付きオンボーディングを再開し、参加完了できる。

### 8.7 P5: アプリ未インストール / アカウントなし
1. `https://.../invite/{id}` タップでWebフォールバックが表示される。
2. TestFlight（公開後はApp Store）へ遷移できる。
3. インストール後初回起動で新規登録できる。
4. `招待された方はこちら` からリンク貼り付けで招待コンテキストを復元できる。
5. 招待導線オンボーディングを経由して参加完了できる。

### 8.8 P6: 無効招待（存在しない/期限切れ）
1. 無効な `inviteId` で招待リンクを開くと、`招待リンクエラー` 画面へ遷移する。
2. `pending_invite_id` がクリアされ、通常導線に影響を残さない。
3. `トップへ戻る` でルート画面へ戻れる。

### 8.9 P7: 既に別チーム所属
1. 既に別チームへ所属済みのユーザーが招待リンクを開き、参加操作を行う。
2. 参加処理で `alreadyHasFamily` を検出し、エラーメッセージが表示される。
3. 既存のチーム所属状態が変わらない。

## 9. セキュリティメモ
- セキュリティ懸念と後対応タスクは以下を参照。
- `docs/security-risk-memo.md`

## 10. E2Eテストケース（追加）
1. P1a: 招待リンク受信後、`参加してはじめる` 押下で招待付きオンボーディングへ遷移する。
2. P1a: オンボ最終ステップ連打時に二重参加が発生しない（参加処理が1回のみ）。
3. P1b: `参加してはじめる` → ログイン画面遷移後に認証し、招待付きオンボーディングへ復帰する。
4. P1b: ログイン前にアプリを強制終了して再起動しても、招待IDが復元されてフローを継続できる。
5. P1a/P1b: 期限切れリンクで専用エラー画面へ遷移し、再試行しても無効招待として扱われる。
6. P1a/P1b: 通信失敗時は招待IDを保持し、通信回復後に再試行で参加完了できる。

## 11. 無効招待のベスト対応タスク
1. エラー画面の行動導線を強化する（`招待した人に再発行を依頼` 文言と `招待リンクを貼り直す` 導線）。
2. 無効理由を分類して表示する（`見つからない` / `期限切れ`）一方、内部情報は出しすぎない。
3. 無効招待発生を計測する（イベント: `invite_invalid_not_found` / `invite_invalid_expired`）し、流入元別に分析可能にする。
4. 再発防止のため、招待発行側に「期限切れ前リマインド」と「ワンタップ再発行」を追加する。
5. サポート運用を整備する（問い合わせテンプレート、確認項目、再発行手順）。
